...
function               GET_VOTERS
		  ( p_required in number, -- Required number of jurors
            p_minDate  in varchar2,   -- Not over 70 yrs old
			p_maxDate	in varchar2 ,  -- Not less than 18yrs old
...
--            p_minDate  in varchar2,  -- Not over 70 yrs old ('yyyy-mm-dd')
--  		  p_maxDate	in varchar2 ,  -- Not less than 18yrs old ('yyyy-mm-dd')
...
--  V1.3    A Rahman  03/10/22 Updated to work on new Voters table with same parameters
-----------------------------------------------------------------------------------
    as
	  ...
	  tab_rowid t_rowid;
	  l_julian_min_dt number := to_number(to_char(to_date(p_minDate, 'yyyy-mm-dd'),'J')); -- Since the input date format cannot be guaranteed  across varios systems,
  	  l_julian_max_dt number := to_number(to_char(to_date(p_maxDate, 'yyyy-mm-dd'),'J')); -- I am changing it to Julian Format.
	  l_found boolean;
  begin
  ...
  -- RFC1731 non coroner pool rules...other rules apply for coroner pools....
	   open l_cursor for ' select rowidtochar(rowid) row_id, flags '||
             ...
             ' and ( FLAGS is null OR '''||p_pool_type||'''  = ''N'' )' ||               -- only coroner pools check flag v1.1 <<
