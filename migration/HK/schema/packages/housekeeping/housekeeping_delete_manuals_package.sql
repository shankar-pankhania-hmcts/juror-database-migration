...
CREATE OR REPLACE PROCEDURE hk.housekeeping_delete_manuals (p_part_no text, p_owner text) AS $body$
DECLARE
ora2pg_rowcount int;
BEGIN

     PERFORM set_config('housekeeping.l_error_stage', 'MANUALS', false);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE hk.housekeeping_delete_manuals (p_part_no text, p_owner text) FROM PUBLIC;
