...
SET search_path = hk,public;
   ...
   --
   -----------
CREATE OR REPLACE PROCEDURE hk.housekeeping_write_footer (p_code integer) AS $body$
BEGIN

     utl_file.put_line(l_file,' ',TRUE);
     utl_file.put_line(l_file,'No. of Juror selected for deletion ,'||current_setting('housekeeping.l_part_count')::integer,TRUE);
     utl_file.put_line(l_file,'No. of Juror successfully deleted  ,'||l_cursor_loops,TRUE);
     utl_file.put_line(l_file,'No. of failures                    ,'||current_setting('housekeeping.l_pool_errors')::integer,TRUE);
     ...
     ,      jurors_error   = current_setting('housekeeping.l_pool_errors')::integer
     WHERE  seq_id = l_seq;
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE hk.housekeeping_write_footer (p_code integer) FROM PUBLIC;
