...
SET search_path = hk,public;
   ...
   --
   -----------
CREATE OR REPLACE PROCEDURE hk.housekeeping_nocommit_delete_sa_attendance () AS $body$
DECLARE
...
BEGIN
     ...
     WHERE  month_start + current_setting('housekeeping_nocommit.l_param_court_threshold')::integer < clock_timestamp();

     GET DIAGNOSTICS ora2pg_rowcount = ROW_COUNT;


     utl_file.put_line(l_file,'ATTENDANCE,'|| ora2pg_rowcount||','||TO_CHAR(l_start,'DD-MM-YYYY hh24:MI:SS')||','||TO_CHAR(clock_timestamp(),'DD-MM-YYYY hh24:MI:SS'),TRUE);
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE hk.housekeeping_nocommit_delete_sa_attendance () FROM PUBLIC;
