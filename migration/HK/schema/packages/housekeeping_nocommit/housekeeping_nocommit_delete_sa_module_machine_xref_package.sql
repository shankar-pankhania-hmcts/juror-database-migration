...
CREATE OR REPLACE PROCEDURE hk.housekeeping_nocommit_delete_sa_module_machine_xref () AS $body$
DECLARE
...
BEGIN
     ...
     utl_file.put_line(l_file,'MODULE_MACHINE_XREF,'|| ora2pg_rowcount||','||TO_CHAR(l_start,'DD-MM-YYYY hh24:MI:SS')||','||TO_CHAR(clock_timestamp(),'DD-MM-YYYY hh24:MI:SS'),TRUE);

     DELETE FROM module_machine_xref
     WHERE  last_update + current_setting('housekeeping_nocommit.l_param_court_threshold')::integer < clock_timestamp();
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE hk.housekeeping_nocommit_delete_sa_module_machine_xref () FROM PUBLIC;
