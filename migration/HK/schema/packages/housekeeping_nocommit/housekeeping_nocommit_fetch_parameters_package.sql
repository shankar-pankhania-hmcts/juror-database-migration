...
CREATE OR REPLACE PROCEDURE hk.housekeeping_nocommit_fetch_parameters () AS $body$
DECLARE
...
BEGIN
     ...
     FOR l_hk_params IN c_hk_params LOOP
       ...
       IF l_hk_params.key = 1 THEN
         PERFORM set_config('housekeeping_nocommit.l_param_court_threshold', l_hk_params.value, false);
       ELSIF l_hk_params.key = 2 THEN
         PERFORM set_config('housekeeping_nocommit.l_param_bureau_threshold', l_hk_params.value, false);
       ELSIF l_hk_params.key = 3 THEN
         PERFORM set_config('housekeeping_nocommit.l_param_max_deletes', l_hk_params.value, false);
       END IF;
     END LOOP;

     housekeeping_nocommit_write_params_to_log;

     -- raise error if database stored parameters are not all present
     IF current_setting('housekeeping_nocommit.l_param_court_threshold') = '' OR current_setting('housekeeping_nocommit.l_param_bureau_threshold') = '' OR current_setting('housekeeping_nocommit.l_param_max_deletes') = '' THEN
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE hk.housekeeping_nocommit_fetch_parameters () FROM PUBLIC;
