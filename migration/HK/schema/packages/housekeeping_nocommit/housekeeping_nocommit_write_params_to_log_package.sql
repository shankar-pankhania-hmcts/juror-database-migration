...
CREATE OR REPLACE PROCEDURE hk.housekeeping_nocommit_write_params_to_log () AS $body$
DECLARE
...
BEGIN
      ...
      utl_file.put_line(l_file,'Parameter P_COURT_AGE_THRESHOLD ,'||current_setting('housekeeping_nocommit.l_param_court_threshold')::integer,TRUE);
      utl_file.put_line(l_file,'Parameter P_BUREAU_AGE_THRESHOLD,'||current_setting('housekeeping_nocommit.l_param_bureau_threshold')::integer,TRUE);
      ...
      utl_file.put_line(l_file,'Parameter P_MAX_DELETES         ,'||current_setting('housekeeping_nocommit.l_param_max_deletes')::integer,TRUE);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE hk.housekeeping_nocommit_write_params_to_log () FROM PUBLIC;
