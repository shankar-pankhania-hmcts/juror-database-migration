...
************************************************************************/
CREATE OR REPLACE PROCEDURE pkg_single_pool_transfer_transfer_court_unique_pool (p_pool_no text, p_location_code text) AS $body$
DECLARE
...
Exception
	when others then
		CALL pkg_single_pool_transfer_write_error_message('POOL TRANSFER','Error in TRANSFER_COURT_UNIQUE_POOL Package. '||SUBSTR(SQLERRM, 1, 100));
		rollback;
		raise;
END;
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE pkg_single_pool_transfer_transfer_court_unique_pool (p_pool_no text, p_location_code text) FROM PUBLIC;
