...
BEGIN
	...
	BEGIN
	    ...
	    SELECT juror_mod.check_time_expired(p_juror_number,p_start_time_int,p_max_timeout) INTO v_timed_out;
		IF v_timed_out THEN
		...
		DELETE FROM juror_mod.staff_juror_response_audit sjra WHERE sjra.juror_number = p_juror_number;
		-- check if timeout has elapsed - if so exit loop
	    SELECT juror_mod.check_time_expired(p_juror_number,p_start_time_int,p_max_timeout) INTO v_timed_out;
		IF v_timed_out THEN
			p_print_msg := 'DELETE FAILED - ERROR:-> TIMED OUT';
